...
http.createServer((req, res) => {
    ...
    if (req.method === 'POST') {
        ...
        req.on('end', () => {
            ...
            res.writeHead(200, { 'Content-Type': 'text/html' });
            res.end(`
                <h1 style="color: green; text-decoration:none;">Result</h1>
                <p>${result}</p>
                <button onclick="window.location.href='/'" style="color: green; background-color: #CCFFCC; border: 1px; padding: 10px 15px; font-weight:bold; cursor: pointer;">
                    Go Back
                </button>
            `);
